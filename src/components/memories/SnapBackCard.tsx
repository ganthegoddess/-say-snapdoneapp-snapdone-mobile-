...
export function SnapBackCard({ memory, recallReason, onArchive }: SnapBackCardProps) {
  ...
  // Track snapback_delivered on mount
  useEffect(() => {
    trackEvent("snapback_delivered", {
      category: action.action_type || "other",
      time_since_capture_min: original.captured_at
        ? Math.round((Date.now() - new Date(original.captured_at).getTime()) / 60000)
        : undefined,
    });
  }, []);
  ...
  return (
    <View style={styles.card}>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          trackEvent("snapback_opened", { category: action.action_type || "other" });
      ...
      {onArchive ? (
        <TouchableOpacity onPress={() => {
          trackEvent("snapback_dismissed", { category: action.action_type || "other" });
          onArchive();
